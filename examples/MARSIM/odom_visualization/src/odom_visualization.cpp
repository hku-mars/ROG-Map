...
void odom_callback(const nav_msgs::Odometry::ConstPtr &msg) {
    ...
    if (isnan(msg->pose.pose.orientation.w) || isnan(msg->pose.pose.orientation.x) ||
        isnan(msg->pose.pose.orientation.y) || isnan(msg->pose.pose.orientation.z)) {
        return;
    }
    ...
    poseROS.pose.position.x = msg->pose.pose.position.x;
    poseROS.pose.position.y = msg->pose.pose.position.y;
    poseROS.pose.position.z = msg->pose.pose.position.z;
//    q = R_to_quaternion(ypr_to_R(pose.rows(3, 5)));
    poseROS.pose.orientation.w = 1.0; //q(0);
    poseROS.pose.orientation.x = 0.0; //q(1);
    poseROS.pose.orientation.y = 0.0; //q(2);
    poseROS.pose.orientation.z = 0.0; //q(3);
...
//    poseROS.pose.orientation.z = q(3);
    ...
    if (color_r > 0.999 && color_g > 0.999 && color_b > 0.999) {
